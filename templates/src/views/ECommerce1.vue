<template>
  ...
  <MDBContainer class="my-5 py-5">
    ...
    <section>
      <MDBRow>
        <MDBCol md="4" class="mb-4">
          <MDBCard>
            <MDBCardBody>
              <MDBSelect v-model:options="selectProduct" label="Product" />
            </MDBCardBody>
          </MDBCard>
        </MDBCol>
            ...
            <MDBCardBody>
              <MDBRow>
                <MDBCol md="6" class="mb-4 mb-md-0">
                  <MDBSelect v-model:options="selectDate" label="Date" />
                </MDBCol>
    ...
    <section class="mb-4">
      <MDBCard>
        <MDBCardHeader class="text-center py-3 bg-light border-0">
    ...
    <section class="mb-4">
      <MDBRow>
        <div class="col-md-5 mb-4">
          <MDBCard>
            <MDBCardHeader class="text-center border-0 bg-light py-3">
    ...
    <section>
      <MDBCard>
        <MDBCardHeader bg="light" border="0" class="p-3 text-center">
        ...
        <MDBCardBody>
          <MDBRow>
            ...
            <MDBCol md="8" class="mb-4">
              <MDBDatatable :dataset="datasetRevenue"></MDBDatatable>
            </MDBCol>
          </MDBRow>
        </MDBCardBody>
      </MDBCard>
  ...
  setup() {
    ...
    const dataset = ref({
      ...
      rows: [
        ["Norway", "$72.63", "8", "No"],
        ["Barbados", "$81.52", "4", "No"],
        ["Saint Pierre and Miquelon", "$32.02", "3", "No"],
        ["Burkina Faso", "$53.21", "6", "No"],
        ["Korea, South", "$3.93", "6", "Yes"],
        ["Finland", "$31.58", "9", "Yes"],
        ["Mayotte", "$75.94", "5", "No"],
        ["Netherlands", "$31.21", "10", "Yes"],
        ["Slovenia", "$24.68", "10", "Yes"],
      ],
    });
    ...
    const shoppingFunnelChartOptions = ref({
      indexAxis: "y",
      scales: {
        x: {
          stacked: true,
      ...
      plugins: {
        ...
        datalabels: {
          formatter: (value) => {
            ...
            let dataArr = shoppingFunnelChartData.value.datasets[0].data;
            dataArr.map((data) => {
              sum += data;
            });
    ...
    const shoppingFunnelChartData = ref({
    ...
    const productRevenueChartOptions = ref({
      scales: {
        y: {
          ticks: {
            display: false,
    ...
    const productRevenueChartData = ref({
      labels: [
        "Monday",
        "Tuesday",
        "Wednesday",
    ...
    const revenueSourcesChartOptions = ref({
      scales: {
        y: {
          ticks: {
            beginAtZero: true,
      ...
      plugins: {
        ...
        datalabels: {
          formatter: (value) => {
            let sum = 0;
            let dataArr = revenueSourcesChartData.value.datasets[0].data;
            dataArr.map((data) => {
              sum += data;
            });
            let percentage = ((value * 100) / sum).toFixed(2) + "%";
    ...
    const revenueSourcesChartData = ref({
    ...
    const marketingSourcesChartOptions = ref({
      plugins: {
        legend: {
          position: "right",
          labels: {
        ...
        datalabels: {
          formatter: (value) => {
            let sum = 0;
            let dataArr = marketingSourcesChartData.value.datasets[0].data;
            dataArr.map((data) => {
              sum += data;
            });
            let percentage = ((value * 100) / sum).toFixed(2) + "%";
    ...
    const marketingSourcesChartData = ref({
